...
namespace Odessa
{
    public partial class OprMap : Form
    {
        private OprClassMap m; //object for work with maps
        ...
        private void button7_Click_1(object sender, EventArgs e)
        {
           ...
           // m.SaveToDB(list_calc[cbIssue.SelectedIndex][1].ToString(),
           //      list_params[cbParams.SelectedIndex][0].ToString(), id_of_exp.ToString(), dataGridView1);
           //else
                m.SaveToDB((cbIssue.SelectedItem as Issue).id.ToString(),
        ...
        private void button9_Click(object sender, EventArgs e)
        {
            ...
            if (cbIssue.SelectedItem is Issue)
                m.highlight_polygon_from_table((cbIssue.SelectedItem as Issue).id.ToString(), id_param,
                 Convert.ToInt16(gMapControl.Zoom.ToString()), id_of_exp);
            else
                MessageBox.Show("Оберіть issue!");
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (cbIssue.SelectedIndex >= 0)
            {
                ...
                MessageBox.Show("Оберіть серію задачу, для яких бажаєте побудувати зону забруднення!");
        }
        ...
        {
            m = new OprClassMap(this.gMapControl); //создаем экземпляр класса
            m.load_map(5); //начальная прогрузка карты с масштабом 5
            btnEndPoint.Enabled = false;
            //cbIssue.Items.Add("LOL");
            m.FillFormulas(id_of_exp, cbIssue);
            ...
            //    cbIssue.Items.Add(list_calc[i][0].ToString());
        ...
        private void cbCalc_SelectedIndexChanged(object sender, EventArgs e)
        {
            if(cbIssue.SelectedIndex>=0)
                ...
                string condition = "calculations_description.calculation_name ='" + cbIssue.Text + 
